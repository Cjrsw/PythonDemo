...
class类名(父类一,父类二,...):
    pass
```
当继承足够多类的时候不需要新的属性和方法，但是又必须满足语法，用*pass*关键字
表示空内容
### 复写
在子类重新定义父类相同命名的方法，属性，就叫做父类的重写
重写后调用父类可以通过：
1. 父类名.属性
...
2. super().属性
    ...
    super().方法()#不用写self
### 类型注解
方便第三方工具和开发者对代码进行类型注释
ctrl p可以查看属性的类型

类似泛型但是不强制，仅仅只是提示，不是决定性的
1. 对变量类型注解
    ...
    b:Student=Student()#基础对象类型

    c:list=[1,3,4]#基础容器类型
    
    d:tuple[str,int,bool]=("cjr",1,True)#详细容器类型
    
    e:dict[str,int]={"cjr":666}
...
```
### 多态
同样行为（函数）传入不同对象，得到不同的状态

见案例
父类方法pass相当于抽象类，只是定义顶层设计，便于子类具体实现重写方法